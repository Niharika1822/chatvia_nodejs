...
io.on("connection", (socket) => {
                    ...
                    setTimeout(() => {
                      contactList(contact.created_by).then((contacts) => {
                        contacts.forEach((contact) => {
                          ...
                          lastMsg(contact.created_by, element.user_id).then(
                            (res) => {
                              for (const key in users) {
                                if (contact.created_by == users[key]) {
                                  io.to(key).emit("isMessage", {
                                    messages: res,
                                  });
                                }
                              }
  ...
  socket.on(
    "chat message",
    function ({ message, sender_id, receiver_id, file_upload, flag }) {
      const single_message = new Msg({
        message,
        sender_id,
        receiver_id,
        file_upload,
  ...
  socket.on(
    "message_update",
    ({ messageId, message, receiverId, userId, flag }) => {
  ...
  socket.on(
    "createGroups",
    function ({ name, description, contact_list, userId }) {
  ...
  socket.on(
    "group message",
    function ({ message, sender_id, group_id, file_upload }) {
